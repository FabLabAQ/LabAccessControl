...
#define DEBUG_FLAG "REDIR"
#include "../../include/la_debug.h"
...
bool HTTPSRedirect::printRedir(void)
{
	...
	if (!connected())
	{
		LA_DPRINTF("Error! Not connected to host.\n");
		return false;
	}
	...
	while (connected())
	{
		...
		case 302:
		{
			...
			if (getLocationURL())
			{
				...
				{
					LA_DPRINTF("Connection to re-directed URL failed!\n");
					return false;
				}
			...
			{
				LA_DPRINTF("Unable to retrieve redirection URL!\n");
				return false;
			}
		}
		break;
		...
		default:
			LA_DPRINTF("Error with request. Response status code: %u\n", httpStatus);
			return false;
			break;
		} // end of switch
...
bool HTTPSRedirect::getLocationURL(void)
{
	...
	{
		LA_DPRINTF("No valid 'Location' field found in header!");
	}

	// Create a GET request for the new location
	createGetRequest(_redirUrl, _redirHost.c_str());

	LA_DPRINTF("_redirHost: %s _redirUrl: %s\n", _redirHost.c_str(), _redirUrl.c_str());
...
unsigned int HTTPSRedirect::getResponseStatus(void)
{
	...
	{
		LA_DPRINTF("Error! No valid Status Code found in HTTP Response.\n");
		statusCode = 0;
		reasonPhrase = "";
	}
...
bool HTTPSRedirect::reConnectFinalEndpoint(void)
{
	...
	if (!connect(_redirHost.c_str(), _httpsPort))
	{
		LA_DPRINTF("Connection to final URL failed!\n");
		return false;
	}
...
{
	LA_DPRINTF("Transfer Encoding: %s Content Length: %u Content Type: %s\n", _hF.transferEncoding.c_str(), _hF.contentLength, _hF.contentType.c_str());
}
#endif
